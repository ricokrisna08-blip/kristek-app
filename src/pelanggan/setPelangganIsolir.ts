...
export type SetPelangganIsolirResult =
  | { success: true }
...
export async function setPelangganIsolir(
  client: SupabaseClient,
  pelangganId: string,
  isolir: boolean
): Promise<SetPelangganIsolirResult> {
  const { data, error } = await withTimeout(
    client.functions.invoke("mikrotik-set-isolir", {
      body: { pelangganId, isolir },
    }),
    INVOKE_TIMEOUT_MS
  ).catch((err: Error) => ({
    data: null,
